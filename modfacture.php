<html>
<head>
	<title>Ajout Client</title>
	<link rel="stylesheet" type="text/css" href="css/client.css">
	<link rel="stylesheet" type="text/css" href="css/accuil.css">
    <link rel="stylesheet" type="text/css" href="css/facture.css">
</head>
<body>

<!-- ********************************************************************************************** -->

<div class="fou9">
	  <div class="nom">
	   <a href="acceuil.html"><h1>MN<span>COMMERCE</span></h1></a> 
    </div>
     <div class="sousnom">Application gestion commercial</div>

      <div class="janbe">
            <div class="user">
                <span>Juan Dela Cruz</span>
            </div> 
      </div> 
</div>
<div class="khate"></div>

<!-- *********************************************************************************************** -->
<div class="left3">
<div id="conteneur-menu2">
<ul>
    <li class="aj"><a href="facture.html">Ajouter facture</a></li>
    <li class="mod"><a href="modfacture.html">Modifier facture</a></li>
    <li class="sup"><a href="supfacture.html">Supprimer facture</a></li>
    <li class="aff"><a href="afffacture.html">Affichier les factures</a></li>
    <li class="rech"><a href="rechfacture.html">Rechercher un facture</a></li>

</ul>  
</div>
<img class="image" src="img/fac.png">
</div>
<!-- *********************************************************************************************** -->
<div class="right3">
	
    <form class="stdform stdform2" method="post" action="#">
                         <p>
                            <label>Recherche  :</label>
                            <span class="field"><input type="text" name="rech" id="rech" class="longinput" /></span>
                        </p>
                        <button class="rech">recherche</button>
                        
                        <p>
                            <label>Facture N°  :</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>
                         <p>
                            <label>N de livraison  :</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>
                          <p>
                            <label>Societe</label>
                            <span class="field">
                            <select name="selection" id="selection">
                                <option value="">Choose One</option>
                                <option value="1">Selection One</option>
                                <option value="2">Selection Two</option>
                                <option value="3">Selection Three</option>
                                <option value="4">Selection Four</option>
                            </select>
                            </span>
                        </p>

                         <p>
                            <label>Produit</label>
                            <span class="field">
                            <select name="selection" id="selection">
                                <option value="">Choose One</option>
                                <option value="1">Selection One</option>
                                <option value="2">Selection Two</option>
                                <option value="3">Selection Three</option>
                                <option value="4">Selection Four</option>
                            </select>
                            </span>
                        </p>

                        <p>
                            <label>Quantite  :</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>

                         <p>
                            <label>Prix de vente HT :</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>


                         <p>
                            <label>Total HT :</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>


                         <p>
                            <label>Remise % :</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>
                         
                         <p>
                            <label>Prix remise :</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>

                         <p>
                            <label>TVA %:</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>

                         <p>
                            <label>Totale TTC:</label>
                            <span class="field"><input type="text" name="email" id="email2" class="longinput" /></span>
                        </p>

                           <p>
                            <label>Reglement :</label>
                            <span class="field">
                                <input type="radio" name="radiofield" /> Espece &nbsp; &nbsp;
                                <input type="radio" name="radiofield" /> Cheque &nbsp; &nbsp;
                                <input type="radio" name="radiofield" /> Virement &nbsp; &nbsp;
                               
                            </span>
                        </p>
                        
                        <p class="stdformbutton">
                            <button class="submit radius2">Imprimer</button>
                            <input type="reset" class="reset radius2" value="Reset Button" />
                        </p>
    </form>


</div>








<!-- ************************* -->
<footer class="ta7t">
   <ul>
    <li><a href="">Client</a></li>
    <li><a href="">Fournisseur</a></li>
    <li><a href="">Produit</a></li>
    <li><a href="">Event</a></li>
    <li><a href="">Analytics</a></li>
   </ul>
 </footer>
<!-- ************************* -->

</body>
</html>